...
% 系统
\node[draw, rectangle, minimum width=2.5cm, minimum height=1.2cm] (system) at (15,0.5) {Markdown\\渲染引擎};
...
% 节点
\node[draw, rectangle, minimum width=3cm, minimum height=1.2cm] (ui) {用户界面\\UI Layer};
\node[draw, rectangle, minimum width=3cm, minimum height=1.2cm, below of=ui] (model) {业务逻辑\\WorkingNote};
\node[draw, rectangle, minimum width=3cm, minimum height=1.2cm, below of=model] (provider) {数据访问\\NotesProvider};
\node[draw, rectangle, minimum width=3cm, minimum height=1.2cm, below of=provider] (db) {数据存储\\SQLite};

\node[draw, rectangle, minimum width=3cm, minimum height=1.2cm, right of=provider, xshift=4cm] (sync) {云同步\\Google Tasks};
