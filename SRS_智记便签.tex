...
\setCJKfamilyfont{songti}{FandolSong-Regular}[AutoFakeBold]
...
\begin{document}
	...
	\begin{titlepage}
		...
		% --- 2号宋体 (22pt) ---
		% 我们用 \CJKfamily{songti} 临时切换到宋体
		{\CJKfamily{songti}\fontsize{22}{26}\selectfont 计算机科学与技术学院}\\[2cm]
		
		% --- 小1号黑体 (24pt), 粗体 ---
		% 这会使用您的 \setCJKmainfont 默认字体 (黑体)
		{\fontsize{24}{28}\selectfont\bfseries 智记便签 项目}\\[1cm]
		{\fontsize{24}{28}\selectfont\bfseries 软件需求规格说明}\\[4cm]
		
		% --- 4号宋体 (14pt) ---
		\begin{center}
			% 再次切换到宋体，并设置4号字大小 (14pt)
			{\CJKfamily{songti}\fontsize{14}{18}\selectfont
				\begin{tabular}{l@{\hspace{1cm}}l}
					% --- 第一位学生 ---
					% 注意：这里不再使用 \Large，而是统一服从上面的 4号(14pt) 字体设置
					...
					% --- 其他信息 ---
					\makebox[4cm][s]{人员分工：} & \makebox[8cm][l]{潘铮负责画图与组织SRS的架构，季凯阳负责具体内容编写} \\[1.2cm]
					\makebox[4cm][s]{合作方法：} & \makebox[8cm][l]{线下沟通，线上飞书互传文件} \\[2cm]
	...
	本文档详细说明了智记便签（Smart Note）软件的功能需求、性能需求、设计约束和质量属性。智记便签是在小米便签基础上进行功能增强和优化的移动端便签应用。
	...
	\begin{itemize}[leftmargin=2cm]
		...
		\item \textbf{扩展流程}：
		\begin{enumerate}
			\item[4a.] 如果选择HTML格式，系统使用Markdown渲染库将文本渲染为HTML字符串
	...
	\begin{figure}[h]
		...
		\begin{tikzpicture}[scale=0.65, transform shape]
			% FormatConverter类
			\node[draw, rectangle, minimum width=5cm, align=left] (converter) at (0,4) {
			...
			% ShareHelper类
			\node[draw, rectangle, minimum width=5cm, below of=converter, yshift=-5.5cm, align=left] (helper) {
				\textbf{ShareHelper} \\
				\rule{5cm}{0.4pt} \\
				-context: Context \\
				\rule{5cm}{0.4pt} \\
			...
			% FormatDialog类
			\node[draw, rectangle, minimum width=5cm, right of=converter, xshift=9cm, align=left] (dialog) {
				\textbf{FormatDialog} \\
				\rule{5cm}{0.4pt} \\
	...
	\begin{itemize}[leftmargin=2cm]
		...
		\item \textbf{资源占用}：
		\begin{itemize}
			\item 应用内存占用：正常使用时保持较低水平
			\item 数据库大小：支持大量便签存储，占用空间合理
			\item APK安装包大小：控制在合理范围内
		\end{itemize}
		\item \textbf{并发性}：支持多个便签同时打开（通过桌面小部件）
		\item \textbf{容量}：支持大量便签存储
	\end{itemize}
	...
	\begin{itemize}[leftmargin=2cm]
		...
		\item 内存占用：峰值小于100MB
		\item 电池消耗：后台待机时保持低功耗
	\end{itemize}
	
	\subsection{可维护性}
	
	\begin{itemize}[leftmargin=2cm]
		\item 代码注释覆盖率：保持适当的注释比例
	...
	\begin{itemize}[leftmargin=2cm]
		...
		\item \textbf{AC-009}：HTML格式在目标应用可正常显示
		\item \textbf{AC-010}：分享功能成功调用系统Intent，支持常见应用
	\end{itemize}
	...
	\begin{itemize}[leftmargin=2cm]
		...
		\item \textbf{PC-004}：应用正常使用内存占用保持合理水平
		\item \textbf{PC-005}：APK安装包大小控制在合理范围
	\end{itemize}
	...
	\begin{itemize}[leftmargin=2cm]
		...
		\item \textbf{CC-002}：适配不同屏幕尺寸（小屏手机、大屏手机、平板）
	...
	\begin{itemize}[leftmargin=2cm]
		\item \textbf{单元测试}：对核心业务逻辑类（WorkingNote、FormatConverter、MarkdownRenderer）进行单元测试，保持较高覆盖率
		...
		\item \textbf{兼容性测试}：在多个不同Android版本的设备上测试
		\item \textbf{用户验收测试（UAT）}：邀请目标用户进行实际使用测试，收集反馈
	\end{itemize}
