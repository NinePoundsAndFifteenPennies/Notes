...
% MarkdownRenderer类
\node[draw, rectangle, minimum width=5cm, align=left, below of=activity, yshift=-8.8cm] (renderer) {
...
% Format枚举
\node[draw, rectangle, minimum width=6cm, below of=dialog, yshift=-5.5cm, align=left] (format) {
...
\item 使用MarkdownRenderer将Markdown渲染为HTML字符串
\item 返回渲染后的HTML内容
\end{enumerate}
...
% NoteData类
\node[draw, rectangle, minimum width=5.5cm, align=left, below of=note, yshift=-7.2cm] (data) {
\textbf{NoteData} \\
\rule{5.5cm}{0.4pt} \\
+ID: String \\
+MIME\_TYPE: String \\
...
\begin{longtable}{|p{5cm}|p{8cm}|} \hline
	\multicolumn{2}{|l|}{其中 $\texttt{baseurl} = \texttt{content://com.xiaomi.notes}$} \\ \hline
	\textbf{URI} & \textbf{说明} \\ \hline
	\texttt{\{\{baseurl\}\}/note} & 访问所有便签 \\ \hline
	\texttt{\{\{baseurl\}\}/note/\#} & 访问特定ID的便签 \\ \hline
	\texttt{\{\{baseurl\}\}/data} & 访问所有数据 \\ \hline
	\texttt{\{\{baseurl\}\}/data/\#} & 访问特定ID的数据 \\ \hline
	\texttt{\{\{baseurl\}\}/note/\#/data} & 访问特定便签的所有数据 \\ \hline
\end{longtable}
